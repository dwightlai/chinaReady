...
export function ForeignCardFailureGuide() {
  return <ArticleSections answer="If a foreign card fails, switch to a prepared backup first. Diagnose the issuer, verification and app setup only when you still have a usable payment path." sections={[
    { title: "Why cards fail", paragraphs: ["A declined purchase can originate with the card issuer, overseas transaction settings, identity verification, the payment app or the merchant acceptance path. The error shown at checkout may not identify the source."] },
