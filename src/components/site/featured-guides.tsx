  ...
  return (
    <div className="mt-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-bold text-[var(--primary)]">Latest preparation updates</p>
          <h2 className="mt-3 max-w-2xl font-[var(--font-display)] text-3xl leading-[1.15] tracking-[-0.03em] sm:text-4xl">Supporting guidance for checker findings.</h2>
        </div>
        <Link className="font-extrabold text-[var(--ink)]" href="/guides">Browse checker support →</Link>
      </div>
      <div className="mt-8 grid gap-px overflow-hidden rounded-[var(--radius-lg)] border border-[var(--line)] bg-[var(--line)] md:grid-cols-3">
        {guides.map((guide) => guide ? (
          <article className="bg-white p-6 sm:p-7" key={guide.slug}>
            ...
            <Link className="mt-6 inline-block text-sm font-extrabold" href={`/guides/${guide.slug}`}>Read supporting guide →</Link>
          </article>
        ) : null)}
      </div>
    </div>
